...
def test_graph_module_record_helpers(tmp_path):
    node_ids = ["N1", "N2", "ORPHAN", "N1"]
    links = [("L1", "N1", "N2"), ("L1", "N2", "MISSING"), ("LOOP", "N1", "N1")]

    assert node_degrees_from_records(node_ids, links) == {
        "N1": 3,
        "N2": 2,
        "ORPHAN": 0,
        "MISSING": 1,
    }
    assert validate_records(node_ids, links) == {
        "orphan nodes": {"ORPHAN"},
        "duplicate nodes": {"N1"},
